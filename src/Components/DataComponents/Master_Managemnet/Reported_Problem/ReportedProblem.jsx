...
function ReportedProblem() {
  ...
  const handleDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.delete(`${process.env.REACT_APP_BASE_URL}/collections/reportedproblem/${id}`)
          .then((res) => {
            Swal.fire(
              "Deleted!",
  ...
  const getData = () => {
    setLoader(true)
    axios.get(`${process.env.REACT_APP_BASE_URL}/collections/reportedproblem?page=${page}&limit=${limit}`)
      .then((res) => {
        setLoader(false)
  ...
  const handleCreate = () => {
    axios.post(`${process.env.REACT_APP_BASE_URL}/collections/reportedproblem`, currentData)
      .then((res) => {
        getData()
      }).catch((error) => { console.log(error) })
  }
  ...
  const handleEditCountry = (id) => {
    axios.put(`${process.env.REACT_APP_BASE_URL}/collections/reportedproblem/${id}`, currentData)
      .then((res) => {
        getData()
      }).catch((error) => { console.log(error) })
  }
            ...
            <table className="w-full  min-w-max caption-bottom text-sm">
              <thead className="[&amp;_tr]:border-b bg-blue-700 ">
                <tr className="border-b transition-colors  text-white hover:bg-muted/50 data-[state=selected]:bg-muted">
                  ...
                  <th className="h-12 px-4 text-left align-middle font-medium text-muted-foreground">Catalog</th>
                  <th className="h-12 px-4 text-left align-middle font-medium text-muted-foreground">CodeGroup</th>
                  <th className="h-12 px-4 text-left align-middle font-medium text-muted-foreground">ProdGroup</th>
                  <th className="h-12 px-4 text-left align-middle font-medium text-muted-foreground">Name</th>
                  <th className="h-12 px-4 text-left align-middle font-medium text-muted-foreground">Short Text For Code</th>
              ...
              <tbody className="[&amp;_tr:last-child]:border-0  ">
                {data?.map((item, index) => (
                  <tr key={item?._id} className="border-b transition-colors  data-[state=selected]:bg-muted">
                    ...
                    <td className="p-4 font- text-md capitalize align-middle whitespace-nowrap">{item?.catalog}</td>
                    <td className="p-4 font- text-md capitalize align-middle whitespace-nowrap">{item?.codegroup}</td>
                    <td className="p-4 font- text-md capitalize align-middle whitespace-nowrap">{item?.prodgroup}</td>
                    <td className="p-4 font- text-md capitalize align-middle whitespace-nowrap">{item?.name}</td>
                    <td className="p-4 font- text-md capitalize align-middle whitespace-nowrap">{item?.shorttextforcode}</td>
                ...
                <div className=" w-[300px] md:w-[500px] lg:w-[700px] border-b border-solid border-blueGray-200 p-3 flex-auto max-h-[400px] overflow-y-auto">
                  ...
                  <div class="grid md:grid-cols-2 md:gap-6 w-full">
                    ...
                    <div className='relative  w-full mb-5 group'>
                      ...
                      <label class="block mb-2 text-sm font-medium text-gray-900 ">CodeGroup</label>
                      ...
                      <input type="text" onChange={(e) => handleFormData('prodgroup', e.target.value)} id="name" value={currentData?.prodgroup} class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-[4px] focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5  dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" />
