...
function ReplacedPartCode() {
  ...
  const handleDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.delete(`${process.env.REACT_APP_BASE_URL}/collections/replacedpartcodes/${id}`)
          .then((res) => {
            Swal.fire(
              "Deleted!",
  ...
  const getData = () => {
    setLoader(true)
    axios.get(`${process.env.REACT_APP_BASE_URL}/collections/replacedpartcodes?page=${page}&limit=${limit}`)
      .then((res) => {
        setLoader(false)
  ...
  const handleCreate = () => {
    axios.post(`${process.env.REACT_APP_BASE_URL}/collections/replacedpartcodes`, currentData)
      .then((res) => {
        getData()
      }).catch((error) => { console.log(error) })
  }
  ...
  const handleEditCountry = (id) => {
    axios.put(`${process.env.REACT_APP_BASE_URL}/collections/replacedpartcodes/${id}`, currentData)
      .then((res) => {
        getData()
      }).catch((error) => { console.log(error) })
  }
            ...
            <table className="w-full  min-w-max caption-bottom text-sm">
              <thead className="[&amp;_tr]:border-b bg-blue-700 ">
                <tr className="border-b transition-colors  text-white hover:bg-muted/50 data-[state=selected]:bg-muted">
                  ...
                  <th className="h-12 px-4 text-left align-middle font-medium text-muted-foreground">Code Group</th>
                  ...
                  <th className="h-12 px-4 text-left align-middle font-medium text-muted-foreground">Sl No.</th>
              ...
              <tbody className="[&amp;_tr:last-child]:border-0  ">
                {data?.map((item, index) => (
                  <tr key={item?._id} className="border-b transition-colors  data-[state=selected]:bg-muted">
                    ...
                    <td className="p-4 font- text-md capitalize align-middle whitespace-nowrap">{item?.code}</td>
                    <td className="p-4 font- text-md capitalize align-middle whitespace-nowrap">{item?.shorttextforcode}</td>
                    <td className="p-4 font- text-md capitalize align-middle whitespace-nowrap">{item?.slno}</td>
