...
const AdminUserType = () => {
  ...
  const handleDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.delete(`${process.env.REACT_APP_BASE_URL}/collections/usertype/${id}`)
          .then((res) => {
            Swal.fire(
              "Deleted!",
  ...
  const getAllData = () => {
    setLoader(true)
    axios.get(`${process.env.REACT_APP_BASE_URL}/collections/usertype?page=${page}&limit=${limit}`)
      .then((res) => {
        setLoader(false)
  ...
  const handleAddData = () => {
    axios.post(`${process.env.REACT_APP_BASE_URL}/collections/usertype`, currentData)
      .then((res) => {
        getAllData()
      }).catch((error) => { console.log(error) })
  }
  ...
  const handleEditData = (id) => {
    axios.patch(`${process.env.REACT_APP_BASE_URL}/collections/usertype/${id}`, currentData)
      .then((res) => {
        getAllData()
      }).catch((error) => { console.log(error) })
  }
    ...
    <tbody className="[&amp;_tr:last-child]:border-0  ">
      {data?.map((i, index) => (
        <tr key={i._id} className="border-b transition-colors  data-[state=selected]:bg-muted">
          ...
          <td className="p-4  text-md capitalize align-middle whitespace-nowrap">{i?.roles}</td>
    ...
    <form onSubmit={(e) => {
      ...
      <div className=" w-[300px] md:w-[500px] lg:w-[700px] border-b border-solid border-blueGray-200 p-3 flex-auto max-h-[400px] overflow-y-auto gap-6">
        ...
        <label class="block mb-2 text-sm font-medium text-gray-900">Select Role</label>
           ...
           <Select variant='soft' className='rounded-[4px] py-2 border' defaultValue={currentData?.roles || ""} onChange={(e, value) => handleFormData('roles', value)}>
