...
const AdminState = () => {
  ...
  const handleDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.delete(`${process.env.REACT_APP_BASE_URL}/collections/state/${id}`)
          .then((res) => {
            Swal.fire(
              "Deleted!",
  ...
  const handleAddData = () => {
    axios.post(`${process.env.REACT_APP_BASE_URL}/collections/state`, currentData)
      .then((res) => {
        getAllData()
      }).catch((error) => { console.log(error) })
  }
  ...
  const handleEditData = (id) => {
    axios.patch(`${process.env.REACT_APP_BASE_URL}/collections/state/${id}`, currentData)
      .then((res) => {
        getAllData()
      }).catch((error) => { console.log(error) })
  }
  ...
  <ModalDialog size='lg' className="p-2 " >
    ...
    <form onSubmit={(e) => {
      ...
      <div className=" w-[300px] md:w-[500px] lg:w-[700px] border-b border-solid border-blueGray-200 p-3 flex-auto max-h-[400px] overflow-y-auto">
        ...
        <div>
             <Select variant='soft' className='rounded-[4px] py-2 border' defaultValue={currentData?.country || ""} onChange={(e, value) => handleFormData('country', value)}>
      ...
      <img src={'https://img.freepik.com/premium-vector/hand-drawn-illustration-lab-assistant_67813-3188.jpg?w=740'}/>
