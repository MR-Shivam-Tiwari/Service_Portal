...
const AdminProductGroup = () => {
  ...
  const handleDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.delete(`${process.env.REACT_APP_BASE_URL}/collections/productgroup/${id}`)
          .then((res) => {
            Swal.fire(
              "Deleted!",
  ...
  const getAllData = () => {
    axios.get(`${process.env.REACT_APP_BASE_URL}/collections/productgroup?page=${page}&limit=${limit}`)
      .then((res) => {
  ...
  const handleAddData = () => {
    axios.post(`${process.env.REACT_APP_BASE_URL}/collections/productgroup`, currentData)
      .then((res) => {
        getAllData()
      }).catch((error) => { console.log(error) })
  }
  ...
  const handleEditData = (id) => {
    axios.patch(`${process.env.REACT_APP_BASE_URL}/collections/productgroup/${id}`, currentData)
      .then((res) => {
        getAllData()
      }).catch((error) => { console.log(error) })
  }
      ...
      <table className="w-full  border  min-w-max caption-bottom text-sm">
        <thead className="[&amp;_tr]:border-b bg-blue-700 ">
          <tr className="border-b transition-colors  text-white hover:bg-muted/50 data-[state=selected]:bg-muted">
            ...
            <th className="h-12 px-4 text-left align-middle font-medium text-muted-foreground">Short Code</th>
            <th className="h-12 px-4 text-left align-middle font-medium text-muted-foreground">CHL No</th>
            <th className="h-12 px-4 text-left align-middle font-medium text-muted-foreground">Rev No</th>
        ...
        <tbody className="[&amp;_tr:last-child]:border-0  ">
          {data?.map((i, index) => (
            <tr key={i._id} className="border-b transition-colors  data-[state=selected]:bg-muted">
              ...
              <td className="p-4  text-md capitalize align-middle whitespace-nowrap">{i?.shortcode}</td>
              <td className="p-4  text-md capitalize align-middle whitespace-nowrap">{i?.ChlNo}</td>
              <td className="p-4  text-md capitalize align-middle whitespace-nowrap">{i?.RevNo}</td>
              <td className="p-4  text-md capitalize align-middle whitespace-nowrap">{i?.type}</td>
          ...
          <div className=" w-[300px] md:w-[500px] lg:w-[700px] border-b border-solid border-blueGray-200 p-3 flex-auto max-h-[400px] overflow-y-auto gap-6">
            ...
            <div class="grid md:grid-cols-2 md:gap-6">
              <div class="relative z-0 w-full mb-5 group">
                ...
                <input onChange={(e) => handleFormData('name', e.target.value)} type="text"  value={currentData?.name} class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-[4px] focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5  dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder=" " required />
              </div>
              <div class="relative z-0 w-full mb-5 group">
                <label class="block mb-2 text-sm font-medium text-gray-900">Short Code	</label>
                <input onChange={(e) => handleFormData('ChlNo', e.target.value)} type="text" value={currentData?.ChlNo} class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-[4px] focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5  dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder=" " required />
              </div>
              <div class="relative z-0 w-full mb-5 group">
                <label class="block mb-2 text-sm font-medium text-gray-900">CHL No	</label>
                <input onChange={(e) => handleFormData('ChlNo', e.target.value)} type="text" value={currentData?.ChlNo} class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-[4px] focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5  dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder=" " required />
              </div>
              <div class="relative z-0 w-full mb-5 group">
                <label class="block mb-2 text-sm font-medium text-gray-900">Rev No	</label>
                <input onChange={(e) => handleFormData('RevNo', e.target.value)} type="text" value={currentData?.RevNo} class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-[4px] focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5  dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder=" " required />
              </div>
              <div class="relative z-0 w-full mb-5 group">
                <label class="block mb-2 text-sm font-medium text-gray-900">Type	</label>
                <input onChange={(e) => handleFormData('type', e.target.value)} type="text" value={currentData?.type} class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-[4px] focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5  dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder=" " required />
              </div>
